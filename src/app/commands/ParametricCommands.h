...
// DCARCRADIUS: DCRADIUS's counterpart for a selected Arc -- pins its
// radius to a typed value via SketchConstraintType::ArcRadius, which
// DocumentConstraints.cpp bridges to ArcEntity::setRadius.
class DcArcRadiusCommand : public DrawCommand {
public:
    DcArcRadiusCommand(Document& document, EntityId arcId) : m_document(document), m_arcId(arcId) {}

    QString start() override { return QStringLiteral("DCARCRADIUS  Radius value:"); }
    std::optional<QString> onPoint(const Point2D& pt) override {
        (void)pt;
        return std::nullopt;
    }
    ...
    std::optional<QString> onText(const QString& text) override {
        ...
        DocumentConstraint constraint;
        constraint.type = SketchConstraintType::ArcRadius;
        constraint.geomA = m_arcId;
        constraint.value = value;
        const DocumentConstraintResult result = solveDocumentConstraints(m_document, {constraint});
        return result.converged
                 ? QStringLiteral("*Radius constraint applied*")
